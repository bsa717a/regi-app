...
async function enforceRateLimit(request: Request) {
  const result = await rateLimit({
    key: clientKeyFromRequest(request, "api:registrations:photos:cover"),
    limit: LIMIT,
    windowMs: WINDOW_MS,
  });
...
/**
 * PATCH /api/registrations/[id]/photos/[photoId]/cover
 */
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    await setRegistrationCoverPhoto({
    ...
    return NextResponse.json(
      { error: "Could not update cover photo." },
