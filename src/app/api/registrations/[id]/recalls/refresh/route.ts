import { NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";
import { getOrCreateUser } from "@/lib/auth/getOrCreateUser";
import {
  clientKeyFromRequest,
  rateLimit,
  rateLimitHeaders,
} from "@/lib/auth/rateLimit";
import { verifyRequest } from "@/lib/auth/verifyRequest";
import { loadRecallsAccess } from "@/lib/recalls/access";
import { loadRecallsOverview } from "@/lib/recalls/loadOverview";
import { refreshRecallsForRegistration } from "@/lib/recalls/sync";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const WINDOW_MS = 60_000;
const LIMIT = 20;

type RouteContext = { params: Promise<{ id: string }> };

async function enforceRateLimit(request: Request) {
  const result = await rateLimit({
    key: clientKeyFromRequest(request, "api:recalls:refresh"),
    limit: LIMIT,
    windowMs: WINDOW_MS,
  });

  if (!result.allowed) {
    return NextResponse.json(
      { error: "Too many requests. Please try again shortly." },
      { status: 429, headers: rateLimitHeaders(result) },
    );
  }

  return result;
}

/** POST /api/registrations/[id]/recalls/refresh — fetch NHTSA and upsert recalls. */
export async function POST(request: Request, context: RouteContext) {
  const limited = await enforceRateLimit(request);
  if (limited instanceof NextResponse) return limited;

  const auth = await verifyRequest(request);
  if (!auth.ok) return auth.response;

  const profile = await getOrCreateUser(auth.decoded);
  const { id } = await context.params;
  const access = await loadRecallsAccess(profile.id, id, { requireEdit: true });
  if (!access.ok) {
    return NextResponse.json(
      { error: access.error },
      { status: access.status, headers: rateLimitHeaders(limited) },
    );
  }

  const refresh = await refreshRecallsForRegistration(access.registration);
  if (!refresh.ok) {
    return NextResponse.json(
      { error: refresh.error },
      {
        status: refresh.ineligible ? 422 : 502,
        headers: rateLimitHeaders(limited),
      },
    );
  }

  const registration = await prisma.registration.findUniqueOrThrow({
    where: { id: access.registration.id },
  });
  const overview = await loadRecallsOverview(registration, access.canEdit);

  return NextResponse.json(
    {
      recalls: overview,
      sync: {
        inserted: refresh.inserted,
        updated: refresh.updated,
        total: refresh.total,
      },
    },
    { headers: rateLimitHeaders(limited) },
  );
}
